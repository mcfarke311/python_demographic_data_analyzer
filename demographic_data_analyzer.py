...
def calculate_demographic_data(print_data=True):
    ...
    highest_earning_country = df.groupby(['native-country']).apply(lambda x: ((x[x.salary == '>50K'].count()) / (x['native-country'].count()))).salary.idxmax()
    highest_earning_country_percentage = round(df.groupby(['native-country']).apply(lambda x: ((x[x.salary == '>50K'].count()) / (x['native-country'].count()))).salary.max() * 100, 1)
